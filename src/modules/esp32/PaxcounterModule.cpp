...
PaxcounterModule::PaxcounterModule()
    ...
    initDeviceList(DEVICE_LIST_CAPACITY);
}
...
void PaxcounterModule::initDeviceList(size_t capacity) {
    device_list_from_libpax.devices = static_cast<pax_device_info_t *>(malloc(capacity * sizeof(pax_device_info_t)));
    if (device_list_from_libpax.devices) {
        device_list_from_libpax.count = 0;
        device_list_from_libpax.capacity = capacity;
    } else {
        device_list_from_libpax.count = 0;
        device_list_from_libpax.capacity = 0;
    }
}


void PaxcounterModule::cleanupDeviceList() {
    if (device_list_from_libpax.devices) {
        free(device_list_from_libpax.devices);
        device_list_from_libpax.devices = nullptr;
    }
    device_list_from_libpax.count = 0;
    device_list_from_libpax.capacity = 0;
}
...
int32_t PaxcounterModule::runOnce() {
    ...
    if (!firstTime) {
        ...
        libpax_init(handlePaxCounterReportRequest, &count_from_libpax, &device_list_from_libpax,
                    Default::getConfiguredOrDefault(moduleConfig.paxcounter.paxcounter_update_interval,
                                                    default_telemetry_broadcast_interval_secs),
                    0);
        libpax_start();
...
void PaxcounterModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
    ...
    libpax_list(&device_list_from_libpax);
